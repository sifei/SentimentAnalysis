...
def build_train(filename):
    ...
    with open("consolidate_features1.csv",'wb') as fp:
        a = csv.writer(fp)
        a.writerows(toCSV)
    #print "train feature Done"
    return toCSV
build_train("consolidate_rand_train_80.csv")
...
def build_test(filename):
    ...
    title = ['Tweet','Sentiment']
    wordList = load_training("consolidate_rand_train_80.csv")
    tokens = get_tokens(wordList)
    title += tokens
    FeatureFreqBi = bigram_Freq("consolidate_rand_train_80.csv",wordList)
    title += FeatureFreqBi
